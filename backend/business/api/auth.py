    ...
    if request.method == "POST":
        ...
        if user:
            ...
            if token_serializer.is_valid():
                ...
                return JsonResponse({"error": "生成Token失败"}, status=500)
        else:
            return JsonResponse(
                {"error": "用户名或密码错误", "ULogin_legal": False}, status=400
            )
    else:
        return JsonResponse(
            {"error": "请求方法错误", "ULogin_legal": False}, status=400
        )
...
def signup(request):
    if request.method == "POST":
        ...
        if user:
            return JsonResponse(
                {"error": "用户名已存在", "userExists": True}, status=400
            )
        else:
...
def manager_login(request):
    if request.method == "POST":
        ...
        if manager:
            ...
            return JsonResponse({"message": "登录成功", "MLogin_legal": True})
        else:
            return JsonResponse(
                {"error": "用户名或密码错误", "MLogin_legal": False}, status=400
            )
    else:
        return JsonResponse(
            {"error": "请求方法错误", "MLogin_legal": False}, status=400
        )
